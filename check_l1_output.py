...
output = loadtxt(root + "MiniNet/out_layer0.txt", Y_SHAPE)
local_output = loadtxt(root + "logs/l1_output.log", (-1,))
...
local_output_arranged = np.empty_like(output)

for i_k in range(0, out_k, tile_k):
    for i_h in range(0, out_h, tile_h):
        for i_w in range(0, out_w, tile_w):
            tile = output[i_h:i_h + tile_h, i_w:i_w + tile_w, i_k:i_k + tile_k]
            tile_size = np.prod(tile.shape)
            local_tile = local_output[local_tile_start:local_tile_start + tile_size].reshape(tile.shape)
            local_output_arranged[i_h:i_h + tile_h, i_w:i_w + tile_w, i_k:i_k + tile_k] = local_tile
            local_tile_start += tile_size
            is_equal_tile = np.array_equal(tile, local_tile)
            if is_equal_tile:
...
if is_equal:
    print("OK: out_layer0.txt and l1_output.log are the same")
else:
    print("ERROR: out_layer0.txt and l1_output.log differ")
    wrong_indices = np.not_equal(local_output_arranged, output)
    print(local_output_arranged[wrong_indices].shape)
    print("L1 output:")
    print(local_output_arranged[wrong_indices])
    print("Golden output:")
    print(output[wrong_indices])
