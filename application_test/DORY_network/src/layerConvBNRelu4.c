...
void layerConvBNRelu4(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+2304;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 32280;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 768;
  dory_dma_memcpy_async(DMA_copy_k);
  dory_dma_barrier(DMA_copy_k);

  DMA_copy_lambda.ext = (uint32_t) l2_W+3072;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 33824;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 768;
  ...
  DMA_copy_x.number_of_2d_copies = 48;
  DMA_copy_x.number_of_1d_copies = 4;
  DMA_copy_x.length_1d_copy = 24;
  dory_dma_memcpy_async(DMA_copy_x);
  dory_dma_barrier(DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 27664) + 0;
  DMA_copy_W.number_of_2d_copies = 96;
  DMA_copy_W.number_of_1d_copies = 1;
  DMA_copy_W.length_1d_copy = 24;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 8 : 48;
      x_tile_size_w   = (_i_w_load+1 == 14)   ? 4 : 4;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 0;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 8 : 48;
      y_tile_size_w   = (_i_w_load+1 == 14)   ? 4 : 4;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 48, 4, 48, 56, 48,  0, 0,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 96, 1*1, 48, 1*1, 48, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 27664) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+2304 + 768*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 32280 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+3072 + 768*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 33824 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 32280 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 33824 + exec_db_act);
    W = (char *) (l1_buffer + 27664 + exec_db_W);
    y = (char *) (l1_buffer + 9224 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 48 : 48;
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 8 : 48;
    x_tile_size_w_exec   = (_i_w_exec+1 == 14)   ? 4 : 4;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 96 : 96;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 8 : 48;
    y_tile_size_w   = (_i_w_exec+1 == 14)   ? 4 : 4;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 48, 4, 96, 56, 96, 0, 0, 0, 0, 0, 0, 4);
      DMA_copy_y.loc = (l1_buffer + 9224) + db_y;
