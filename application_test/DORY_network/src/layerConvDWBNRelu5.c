...
void layerConvDWBNRelu5(
  ...
  im2col = l1_buffer + 34744;
  volatile char *pwt_buffer;
  pwt_buffer = im2col + 1104;
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+432;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 34216;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+1200;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 34480;
  ...
  DMA_copy_x.number_of_2d_copies = 20;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 34064) + 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 6) ? 16 : 16;
      x_tile_size_h   = (_i_h_load+1 == 4)   ? 3 : 20;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 56 : 56;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*4/8;
      x_length_nif_byte = (_i_nif_load+1 == 6)   ? 8 : 8;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 4)   ? 2 : 18;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 20, 56, 16, 56, 96,  2, 2,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 34064) + db_W;
        ...
        DMA_copy_k.loc = (uint32_t) l1_buffer + 34216 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+1200 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 34480 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 34216 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 34480 + exec_db_act);
    W = (char *) (l1_buffer + 34064 + exec_db_W);
    y = (char *) (l1_buffer + 17928 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 6) ? 16 : 16;
    x_tile_size_h_exec   = (_i_h_exec+1 == 4)   ? 3 : 20;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 56 : 56;
    y_tile_size_nof = (_i_nof_exec+1 == 6) ? 16 : 16;
    y_tile_size_h   = (_i_h_exec+1 == 4)   ? 2 : 18;
    y_tile_size_w   = (_i_w_exec+1 == 1)   ? 56 : 56;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*4/8;
    y_length_nof_byte = (_i_nof_exec+1 == 6)   ? 8 : 8;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 18, 56, 16, 56, 96, 0, 0, 0, 0, 0, 0, 4);
      DMA_copy_y.loc = (l1_buffer + 17928) + db_y;
