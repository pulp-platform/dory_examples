...
void layerConvBNRelu6(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+4608;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+5376;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 18448) + 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 3)   ? 8 : 24;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 0;
      y_tile_size_h   = (_i_h_load+1 == 3)   ? 8 : 24;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 24, 4, 96, 56, 96,  0, 0,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 96, 1*1, 96, 1*1, 96, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 18448) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+4608 + 768*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 27672 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+5376 + 768*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 29216 + db_act;
    ...
    lambda = (int64_t *) (l1_buffer + 29216 + exec_db_act);
    W = (char *) (l1_buffer + 18448 + exec_db_W);
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 24, 4, 96, 56, 96, 0, 0, 0, 0, 0, 0, 4);
