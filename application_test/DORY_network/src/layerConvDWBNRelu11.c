...
void layerConvDWBNRelu11(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 31672;
  ...
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 32192;
  ...
  DMA_copy_x.length_1d_copy = 16;
  dory_dma_memcpy_async(DMA_copy_x);
  dory_dma_barrier(DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 31376) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 12544 : 0;
    db_W = !db_state_W ? 144 : 0;
    db_y = !db_state_y ? 3136 : 0;
    db_act = !db_state_W ? 256 : 0;
    exec_db_x = db_state_x ? 12544 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 6) ? 32 : 32;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 6)   ? 16 : 16;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 6) ? 32 : 32;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 28, 28, 32, 28, 192,  1, 1,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 64.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 31376) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+864 + 256*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 31672 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+2400 + 256*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 32192 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 31672 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 32192 + exec_db_act);
    W = (char *) (l1_buffer + 31376 + exec_db_W);
    y = (char *) (l1_buffer + 25096 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 6) ? 32 : 32;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 28 : 28;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 28 : 28;
    y_tile_size_nof = (_i_nof_exec+1 == 6) ? 32 : 32;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 6)   ? 16 : 16;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 14, 14, 32, 14, 192, 0, 0, 0, 0, 0, 0, 4);
      DMA_copy_y.loc = (l1_buffer + 25096) + db_y;
