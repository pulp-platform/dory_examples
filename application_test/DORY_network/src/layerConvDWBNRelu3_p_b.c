...
#include "layerConvDWBNRelu3_p_b.h"
#define VERBOSE_PRINT(...) printf(__VA_ARGS__)

void layerConvDWBNRelu3_p_b(
  void *args
) {
  //////////////////////////////////////////////////////////////////////////
  ...
  im2col = l1_buffer + 17152;
  volatile char *pwt_buffer;
  pwt_buffer = im2col + 336;
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+216;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 16368;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 384;
  dory_dma_memcpy_async(DMA_copy_k);
  dory_dma_barrier(DMA_copy_k);

  DMA_copy_lambda.ext = (uint32_t) l2_W+600;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 16760;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 384;
  ...
  DMA_copy_x.number_of_1d_copies = 112;
  DMA_copy_x.length_1d_copy = 24;
  dory_dma_memcpy_async(DMA_copy_x);
  dory_dma_barrier(DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 16144) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 13440 : 0;
    db_W = !db_state_W ? 216 : 0;
    db_y = !db_state_y ? 2688 : 0;
    db_act = !db_state_W ? 384 : 0;
    exec_db_x = 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 216 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    {
      asm volatile("": : :"memory");
      y_tile_size_h   = (_i_h_load+1 == 1)   ? 2 : 2;
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 56 : 56;
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 96.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 16144) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+216 + 384*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 16368 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+600 + 384*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 16760 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 16368 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 16760 + exec_db_act);
    W = (char *) (l1_buffer + 16144 + exec_db_W);
    y = (char *) (l1_buffer + 13448 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 48 : 48;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 5 : 5;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 112 : 112;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 48 : 48;
    y_tile_size_h   = (_i_h_exec+1 == 1)   ? 2 : 2;
    y_tile_size_w   = (_i_w_exec+1 == 1)   ? 56 : 56;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*4/8;
    y_length_nof_byte = (_i_nof_exec+1 == 1)   ? 24 : 24;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 2, 56, 48, 56, 48, 0, 0, 0, 0, 0, 0, 4);
      DMA_copy_y.loc = (l1_buffer + 13448) + db_y;
