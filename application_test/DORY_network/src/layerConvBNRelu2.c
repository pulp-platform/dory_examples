...
void layerConvBNRelu2(
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+960;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36512;
  ...
  DMA_copy_x.number_of_1d_copies = 60;
  DMA_copy_x.length_1d_copy = 12;
  dory_dma_memcpy_async(DMA_copy_x);
  dory_dma_barrier(DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 34576) + 0;
  DMA_copy_W.number_of_2d_copies = 48;
  DMA_copy_W.number_of_1d_copies = 1;
  DMA_copy_W.length_1d_copy = 12;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 5760 : 0;
    db_W = !db_state_W ? 576 : 0;
    db_y = !db_state_y ? 11520 : 0;
    db_act = !db_state_W ? 384 : 0;
    exec_db_x = db_state_x ? 5760 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 52 : 60;
      ...
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 52 : 60;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 60, 24, 112, 24,  0, 0,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 48, 1*1, 24, 1*1, 24, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 34576) + db_W;
        ...
        DMA_copy_k.loc = (uint32_t) l1_buffer + 35736 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+960 + 384*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36512 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 35736 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 36512 + exec_db_act);
    W = (char *) (l1_buffer + 34576 + exec_db_W);
    y = (char *) (l1_buffer + 11528 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 24 : 24;
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 8 : 8;
    x_tile_size_w_exec   = (_i_w_exec+1 == 2)   ? 52 : 60;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 48 : 48;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 8 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 2)   ? 52 : 60;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 60, 48, 112, 48, 0, 0, 0, 0, 0, 0, 4);
      DMA_copy_y.loc = (l1_buffer + 11528) + db_y;
