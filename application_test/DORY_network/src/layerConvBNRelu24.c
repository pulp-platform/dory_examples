...
void layerConvBNRelu24(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 27256;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 320;
  dory_dma_memcpy_async(DMA_copy_k);
  dory_dma_barrier(DMA_copy_k);

  DMA_copy_lambda.ext = (uint32_t) l2_W+153600;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 27904;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 320;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 11888) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 5376 : 0;
    db_W = !db_state_W ? 7680 : 0;
    db_y = !db_state_y ? 560 : 0;
    db_act = !db_state_W ? 320 : 0;
    exec_db_x = db_state_x ? 5376 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 7680 : 0;
    exec_db_act = db_state_W ? 320 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 3 : 4;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 20) ? 8 : 40;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 7, 4, 384, 7, 384,  0, 0,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 40, 1*1, 384, 1*1, 384, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 11888) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+147456 + 320*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 27256 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+153600 + 320*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 27904 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 27256 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 27904 + exec_db_act);
    W = (char *) (l1_buffer + 11888 + exec_db_W);
    y = (char *) (l1_buffer + 10760 + db_y);
    ...
    x_tile_size_w_exec   = (_i_w_exec+1 == 2)   ? 3 : 4;
    y_tile_size_nof = (_i_nof_exec+1 == 20) ? 8 : 40;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 20)   ? 4 : 20;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 7, 4, 40, 7, 768, 0, 0, 0, 0, 0, 0, 4);
      DMA_copy_y.loc = (l1_buffer + 10760) + db_y;
