...
void __attribute__ ((noinline)) layerConvDWBNRelu3L3(void *args) 
{
  ...
  if(pi_core_id()==0)
  {
    pi_cl_ram_read(hyperram, l3_x, transfer_input, 13440, &buff_req_x1);
  ...
  for(int j=0; j<28; j++) 
  {
    if(pi_core_id()==0)
    {
      ...
      if (j<27)
        pi_cl_ram_read(hyperram, l3_x + shift, transfer_input, 13440, &buff_req_x1);
    }    
    input_t = !input_t;
    transfer_input = input_t ? L2_input_2 : L2_input_1;
    int k = 0;
    ...
    if (j==0)
    {
      unsigned int args[13] = {l3_x,
          ...
          exec_input,
          l2_x_2,
          dory_get_tile_3d(exec_output, 0, 0, k, 2, 56, 48, 56, 48, 0, 0, 0, 0, 0, 0, 4),
          exec_weights,
          l1_buffer,
          hyperram,
          outmult,
    ...
    else if (j==(27))
    {
      unsigned int args[13] = {l3_x,
          ...
          exec_input,
          l2_x_2,
          dory_get_tile_3d(exec_output, j, 0, k, 2, 56, 48, 56, 48, 0, 0, 0, 0, 0, 0, 4),
          exec_weights,
          l1_buffer,
          hyperram,
          outmult,
