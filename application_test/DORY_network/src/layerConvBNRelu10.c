...
void layerConvBNRelu10(
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+19968;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 96 : 96;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 16, 2, 192, 28, 192,  0, 0,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 96, 1*1, 192, 1*1, 192, 0,0,0,0,0,0, 4);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+18432 + 768*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 27672 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+19968 + 768*_i_nof_load;
    ...
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 96 : 96;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 2)   ? 48 : 48;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 2, 96, 28, 192, 0, 0, 0, 0, 0, 0, 4);
