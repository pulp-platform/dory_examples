...
void layerConvBNRelu12(
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 29720;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 1024;
  dory_dma_memcpy_async(DMA_copy_k);
  dory_dma_barrier(DMA_copy_k);

  DMA_copy_lambda.ext = (uint32_t) l2_W+39936;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 31776;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 1024;
  ...
  DMA_copy_x.length_1d_copy = 96;
  dory_dma_memcpy_async(DMA_copy_x);
  dory_dma_barrier(DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 5136) + 0;
  DMA_copy_W.number_of_2d_copies = 128;
  DMA_copy_W.number_of_1d_copies = 1;
  DMA_copy_W.length_1d_copy = 96;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 1) ? 192 : 192;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 3) ? 128 : 128;
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 192 : 192;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 2, 192, 14, 192,  0, 0,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 128, 1*1, 192, 1*1, 192, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 5136) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+36864 + 1024*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 29720 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+39936 + 1024*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 31776 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 29720 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 31776 + exec_db_act);
    W = (char *) (l1_buffer + 5136 + exec_db_W);
    y = (char *) (l1_buffer + 3080 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 192 : 192;
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 6 : 8;
    x_tile_size_w_exec   = (_i_w_exec+1 == 7)   ? 2 : 2;
    y_tile_size_nof = (_i_nof_exec+1 == 3) ? 128 : 128;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 3)   ? 64 : 64;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 128, 14, 384, 0, 0, 0, 0, 0, 0, 4);
