...
void __attribute__ ((noinline)) layerGemm28_lastL3(void *args) 
{
  ...
  L2_weights_1 = l2_W;
  L2_weights_2 = l2_W + 15360 + 0 + 0;
  transfer_weights = L2_weights_1;
  exec_weights = L2_weights_1;  
  // first tile transfer. Weights, k, lambda
  if(pi_core_id()==0)
  {
    pi_cl_ram_read(hyperram, l3_W, transfer_weights, 15360, &buff_req_w1);
  ...
  for(int k=0; k<25; k++) 
  {
    if (k < 24) 
    {
      if(pi_core_id()==0) 
      {
        pi_cl_ram_read(hyperram, (l3_W+(k+1)*15360), transfer_weights, 15360, &buff_req_w1);
      }
    }  
    ...
    if (j==0)
    {
      unsigned int args[13] = {l3_x,
          ...
          exec_input,
          l2_x_2,
          dory_get_tile_3d(exec_output, 0, 0, k, 1, 1, 40, 1, 1000, 0, 0, 0, 0, 0, 0, 32),
          exec_weights,
          l1_buffer,
          hyperram,
          outmult,
    ...
    else if (j==(0))
    {
      unsigned int args[13] = {l3_x,
          ...
          exec_input,
          l2_x_2,
          dory_get_tile_3d(exec_output, j, 0, k, 1, 1, 40, 1, 1000, 0, 0, 0, 0, 0, 0, 32),
          exec_weights,
          l1_buffer,
          hyperram,
          outmult,
