...
void layerConvBNRelu14(
  ...
  DMA_copy_W.stride_2d = 192;
  DMA_copy_W.stride_1d = 192;
  DMA_copy_W.dir = 1;
  ...
  DMA_copy_k.loc = (uint32_t) l1_buffer + 28568;
  DMA_copy_k.number_of_2d_copies = 1;
  DMA_copy_k.number_of_1d_copies = 1;
  DMA_copy_k.length_1d_copy = (uint16_t) 448;
  dory_dma_memcpy_async(DMA_copy_k);
  dory_dma_barrier(DMA_copy_k);

  DMA_copy_lambda.ext = (uint32_t) l2_W+76800;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 29472;
  DMA_copy_lambda.number_of_2d_copies = 1;
  DMA_copy_lambda.number_of_1d_copies = 1;
  DMA_copy_lambda.length_1d_copy = (uint16_t) 448;
  ...
  DMA_copy_x.length_1d_copy = 192;
  dory_dma_memcpy_async(DMA_copy_x);
  dory_dma_barrier(DMA_copy_x);

  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 7056) + 0;
  DMA_copy_W.number_of_2d_copies = 56;
  DMA_copy_W.number_of_1d_copies = 1;
  DMA_copy_W.length_1d_copy = 192;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_W = !db_state_W ? 10752 : 0;
    db_y = !db_state_y ? 448 : 0;
    db_act = !db_state_W ? 448 : 0;
    exec_db_x = db_state_x ? 3072 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 10752 : 0;
    exec_db_act = db_state_W ? 448 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 1) ? 384 : 384;
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 6 : 8;
      x_tile_size_w   = (_i_w_load+1 == 7)   ? 2 : 2;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*4/8;
      x_length_nif_byte = (_i_nif_load+1 == 1)   ? 192 : 192;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 0;
      y_tile_size_h   = (_i_h_load+1 == 2)   ? 6 : 8;
      y_tile_size_w   = (_i_w_load+1 == 7)   ? 2 : 2;
      W_tile_size_nof = (_i_nof_load+1 == 7) ? 48 : 56;
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 384 : 384;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*4*1*1/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 192 : 192;
      // transfer of next input tile in double buffering

      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 2, 384, 14, 384,  0, 0,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 56, 1*1, 384, 1*1, 384, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 7056) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+73728 + 448*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 28568 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+76800 + 448*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 29472 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 28568 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 29472 + exec_db_act);
    W = (char *) (l1_buffer + 7056 + exec_db_W);
    y = (char *) (l1_buffer + 6152 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 384 : 384;
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 6 : 8;
    x_tile_size_w_exec   = (_i_w_exec+1 == 7)   ? 2 : 2;
    y_tile_size_nof = (_i_nof_exec+1 == 7) ? 48 : 56;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 6 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 7)   ? 2 : 2;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*4/8;
    y_length_nof_byte = (_i_nof_exec+1 == 7)   ? 24 : 28;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 56, 14, 384, 0, 0, 0, 0, 0, 0, 4);
