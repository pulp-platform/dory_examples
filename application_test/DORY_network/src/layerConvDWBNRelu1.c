...
void layerConvDWBNRelu1(
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+108;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 36008;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+300;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36272;
  ...
  DMA_copy_W.ext = l2_W;
  DMA_copy_W.loc = (l1_buffer + 35856) + 0;
  ...
  for(iter=0; iter < total_tiles; iter++) {
    ...
    db_x = !db_state_x ? 9856 : 0;
    db_W = !db_state_W ? 72 : 0;
    db_y = !db_state_y ? 8064 : 0;
    db_act = !db_state_W ? 128 : 0;
    exec_db_x = db_state_x ? 9856 : 0;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    if(iter < (total_tiles-1) )
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 13)   ? 5 : 11;
      x_tile_size_w   = (_i_w_load+1 == 1)   ? 112 : 112;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*4/8;
      x_length_nif_byte = (_i_nif_load+1 == 2)   ? 4 : 8;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 13)   ? 4 : 9;
      y_tile_size_w   = (_i_w_load+1 == 1)   ? 112 : 112;
      ...
      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 11, 112, 16, 112, 24,  2, 2,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        DMA_copy_W.loc = (l1_buffer + 35856) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+108 + 128*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 36008 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+300 + 128*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36272 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 36008 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 36272 + exec_db_act);
    W = (char *) (l1_buffer + 35856 + exec_db_W);
    y = (char *) (l1_buffer + 19720 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 2) ? 8 : 16;
    x_tile_size_h_exec   = (_i_h_exec+1 == 13)   ? 5 : 11;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 112 : 112;
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 8 : 16;
    y_tile_size_h   = (_i_h_exec+1 == 13)   ? 4 : 9;
    y_tile_size_w   = (_i_w_exec+1 == 1)   ? 112 : 112;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*4/8;
    y_length_nof_byte = (_i_nof_exec+1 == 2)   ? 4 : 8;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 9, 112, 16, 112, 24, 0, 0, 0, 0, 0, 0, 4);
      DMA_copy_y.loc = (l1_buffer + 19720) + db_y;
