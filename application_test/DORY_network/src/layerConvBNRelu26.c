...
void layerConvBNRelu26(
  ...
  DMA_copy_W.stride_2d = 384;
  DMA_copy_W.stride_1d = 384;
  DMA_copy_W.dir = 1;
  ...
  ////////////////////////////
  DMA_copy_k.ext = (uint32_t) l2_W+294912;
  DMA_copy_k.loc = (uint32_t) l1_buffer + 24712;
  ...
  DMA_copy_lambda.ext = (uint32_t) l2_W+301056;
  DMA_copy_lambda.loc = (uint32_t) l1_buffer + 24784;
...
///////// POSSIBLE BUG FIX!!!!! DB_STATE_Y NOT SWITCHED /////////////
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 1) ? 768 : 768;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 1)   ? 384 : 384;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 192) ? 4 : 4;
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 768 : 768;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*4*1*1/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 384 : 384;
      // transfer of next input tile in double buffering

      DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 7, 4, 768, 7, 768,  0, 0,0, pad_offset_h, pad_offset_w, 0, 4);
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 4, 1*1, 768, 1*1, 768, 0,0,0,0,0,0, 4);
        DMA_copy_W.loc = (l1_buffer + 21632) + db_W;
        DMA_copy_W.number_of_2d_copies = W_tile_size_nof;
        DMA_copy_W.length_1d_copy = W_length_nif_byte;
        dory_dma_memcpy_async(DMA_copy_W);

        DMA_copy_k.ext = (uint32_t) l2_W+294912 + 32*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 24712 + db_act;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 8;
        dory_dma_memcpy_async(DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+301056 + 32*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 24784 + db_act;
    ...
    k = (int64_t *) (l1_buffer + 24712 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 24784 + exec_db_act);
    W = (char *) (l1_buffer + 21632 + exec_db_W);
    y = (char *) (l1_buffer + 21512 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 768 : 768;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 7 : 7;
    x_tile_size_w_exec   = (_i_w_exec+1 == 2)   ? 3 : 4;
    y_tile_size_nof = (_i_nof_exec+1 == 192) ? 4 : 4;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 192)   ? 2 : 2;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {                        
      ...
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 7, 4, 4, 7, 768, 0, 0, 0, 0, 0, 0, 4);
