...
void layerConvDWBNRelu3(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 31900;
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+832;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  12800, // size: dimension of the buffer
  ...
  for(iter=0; iter<8*3*1; iter++) {
    ...
    db_x = !db_state_x ? 12800 : 0;
    db_W = !db_state_W ? 72 : 0;
    db_y = !db_state_y ? 3072 : 0;
    exec_db_x = db_state_x ? 12800 : 0;
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (last_nif_load) ? 8 : 8;
      x_tile_size_h   = (last_h_load)   ? 17 : 25;
      x_tile_size_w   = (last_w_load)   ? 64 : 64;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (last_nif_load)   ? 8 : 8;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (last_h_load)   ? 8 : 12;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 25, 64, 8, 64, 64,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
    ...
    k = (int32_t *) (l1_buffer + 31900 + _i_nof_exec*32);
    lambda = (int32_t *) (l1_buffer + 32160 + _i_nof_exec*32);
    W = (char *) (l1_buffer + 31752 + exec_db_W);
    y = (char *) (l1_buffer + 25604 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (last_nif_exec) ? 8 : 8;
    x_tile_size_h_exec   = (last_h_exec)   ? 17 : 25;
    x_tile_size_w_exec   = (last_w_exec)   ? 64 : 64;
    y_tile_size_nof = (last_nof_exec) ? 8 : 8;
    y_tile_size_h   = (last_h_exec)   ? 8 : 12;
    ...
    pi_cl_team_barrier(0);
      // wait for DMA write/read
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 12, 32, 8, 32, 64, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 25604) + db_y, // loc
