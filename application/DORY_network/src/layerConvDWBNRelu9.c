...
void layerConvDWBNRelu9(
  ...
  // last-tile flags
  int last_nof_load = (9 == 1) ? 1 : 0;
  int last_nif_load = (9 == 1) ? 1 : 0;
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 32314;
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 33342;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  7936, // size: dimension of the buffer
  ...
  l2_W, // ext
  (l1_buffer + 31752) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  279, // size: dimension of matrix of weight * bytes_per_weight
  ...
  31, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  ...
  for(iter=0; iter<9*1*1; iter++) {
    ...
    last_nof_load = (_i_nof_load+1 == 9) ? 1 : 0;
    last_nif_load = (_i_nof_load+1 == 9) ? 1 : 0;
    last_h_load = (_i_h_load+1 == 1) ? 1 : 0;
    last_w_load = (_i_w_load+1 == 1) ? 1 : 0;

    // compute double buffering offsets and update db state
    db_x = !db_state_x ? 7936 : 0;
    db_W = !db_state_W ? 279 : 0;
    db_y = !db_state_y ? 7936 : 0;
    exec_db_x = db_state_x ? 7936 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 279 : 0;
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (last_nif_load) ? 8 : 31;
      ...
      x_length_nif_byte = (last_nif_load)   ? 8 : 31;
      ...
      W_tile_size_nof = (last_nof_load) ? 8 : 31;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 16, 16, 31, 16, 256,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 31.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 31752) + db_W, // loc
    ...
    k = (int32_t *) (l1_buffer + 32314 + _i_nof_exec*124);
    lambda = (int32_t *) (l1_buffer + 33342 + _i_nof_exec*124);
    W = (char *) (l1_buffer + 31752 + exec_db_W);
    y = (char *) (l1_buffer + 15876 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (last_nif_exec) ? 8 : 31;
    x_tile_size_h_exec   = (last_h_exec)   ? 16 : 16;
    x_tile_size_w_exec   = (last_w_exec)   ? 16 : 16;
    y_tile_size_nof = (last_nof_exec) ? 8 : 31;
    ...
    y_length_nof_byte = (last_nof_exec)   ? 8 : 31;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      // wait for DMA write/read
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 16, 31, 16, 256, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 15876) + db_y, // loc
