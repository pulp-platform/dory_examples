...
void layerConvDWBNRelu9(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 33872;
  ...
  for(iter=0; iter<8*1*1; iter++) {
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+2304 + 256*_i_nof_load;
          ...
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+4352 + 256*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 33872 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 33356 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 33872 + exec_db_act);
