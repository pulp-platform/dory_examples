...
void __attribute__ ((noinline)) layerConvBNRelu24L3(void *args) 
{
  ...
  L2_weights_1 = l2_W;
  L2_weights_2 = l2_W + 16384 + 256 + 256;
  transfer_weights = L2_weights_1;
  exec_weights = L2_weights_1;  
  // first tile transfer. Weights, k, lambda
  if(pi_core_id()==0)
  {
    pi_cl_ram_read(hyperram, l3_W, transfer_weights, 16384, &buff_req_w1);
    pi_cl_ram_read(hyperram, l3_W+524288, transfer_weights + 16384, 256, &buff_req_w2);
    pi_cl_ram_read(hyperram, l3_W+532480, transfer_weights + 16384 + 256, 256, &buff_req_w3);
  ...
  for(int k=0; k<32; k++) 
  {
    if (k < 31) 
    {
      if(pi_core_id()==0) 
      {
        pi_cl_ram_read(hyperram, (l3_W+(k+1)*16384), transfer_weights, 16384, &buff_req_w1);
        pi_cl_ram_read(hyperram, l3_W+524288+ (k+1)*256, transfer_weights + 16384, 256, &buff_req_w2);
        pi_cl_ram_read(hyperram, l3_W+532480 + (k+1)*256, transfer_weights + 16384+ 256, 256, &buff_req_w3);
      }
    }  
    ...
    if (j==0)
    {
      unsigned int args[13] = {l3_x,
          ...
          exec_input,
          l2_x_2,
          dory_get_tile_3d(exec_output, 0, 0, k, 4, 4, 32, 4, 1024, 0, 0, 0, 0, 0, 0, 8),
          exec_weights,
          l1_buffer,
          hyperram,
          outmult,
    ...
    else if (j==(0))
    {
      unsigned int args[13] = {l3_x,
          ...
          exec_input,
          l2_x_2,
          dory_get_tile_3d(exec_output, j, 0, k, 4, 4, 32, 4, 1024, 0, 0, 0, 0, 0, 0, 8),
          exec_weights,
          l1_buffer,
          hyperram,
          outmult,
