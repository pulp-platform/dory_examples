...
void layerConvDWBNRelu11(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 384;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+2304;
    copy_k.loc = (uint32_t) l1_buffer + 31596;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 384;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+4352;
    copy_lambda.loc = (uint32_t) l1_buffer + 32368;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  12288, // size: dimension of the buffer
  ...
  48, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  432, // size: dimension of matrix of weight * bytes_per_weight
  ...
  for(iter=0; iter<6*1*1; iter++) {
    ...
    db_x = !db_state_x ? 12288 : 0;
    db_W = !db_state_W ? 432 : 0;
    db_y = !db_state_y ? 3072 : 0;
    db_act = !db_state_W ? 384 : 0;
    exec_db_x = db_state_x ? 12288 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 432 : 0;
    exec_db_act = db_state_W ? 384 : 0;
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 6) ? 16 : 48;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 6)   ? 16 : 48;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 6) ? 16 : 48;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 16, 16, 48, 16, 256,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      {
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 48.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+2304 + 384*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 31596 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+4352 + 384*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 32368 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 31596 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 32368 + exec_db_act);
    W = (char *) (l1_buffer + 30728 + exec_db_W);
    y = (char *) (l1_buffer + 24580 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 6) ? 16 : 48;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 16 : 16;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 16 : 16;
    y_tile_size_nof = (_i_nof_exec+1 == 6) ? 16 : 48;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 6)   ? 16 : 48;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {  
      ...
      {                                       
        ...
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 8, 48, 8, 256, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 24580) + db_y, // loc
