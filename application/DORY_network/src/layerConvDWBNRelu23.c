...
void layerConvDWBNRelu23(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 1408;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+4608;
    copy_k.loc = (uint32_t) l1_buffer + 31340;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 1408;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+8704;
    copy_lambda.loc = (uint32_t) l1_buffer + 34160;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  11264, // size: dimension of the buffer
  ...
  176, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  dory_dma_memcpy_3d_custom_blocking(
  l2_W, // ext
  (l1_buffer + 28168) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  1584, // size: dimension of matrix of weight * bytes_per_weight
  ...
  176, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  ...
  for(iter=0; iter<3*1*1; iter++) {
    ...
    db_x = !db_state_x ? 11264 : 0;
    db_W = !db_state_W ? 1584 : 0;
    db_y = !db_state_y ? 2816 : 0;
    db_act = !db_state_W ? 1408 : 0;
    exec_db_x = db_state_x ? 11264 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 1584 : 0;
    exec_db_act = db_state_W ? 1408 : 0;
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 3) ? 160 : 176;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 3)   ? 160 : 176;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 3) ? 160 : 176;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 8, 176, 8, 512,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      {
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 176.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 28168) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+4608 + 1408*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 31340 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+8704 + 1408*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 34160 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 31340 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 34160 + exec_db_act);
    W = (char *) (l1_buffer + 28168 + exec_db_W);
    y = (char *) (l1_buffer + 22532 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 3) ? 160 : 176;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 8 : 8;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 8 : 8;
    y_tile_size_nof = (_i_nof_exec+1 == 3) ? 160 : 176;
    y_tile_size_h   = (_i_h_exec+1 == 1)   ? 4 : 4;
    y_tile_size_w   = (_i_w_exec+1 == 1)   ? 4 : 4;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 3)   ? 160 : 176;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {  
      ...
      {                                       
        ...
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 4, 4, 176, 4, 512, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 22532) + db_y, // loc
