...
void layerConvDWBNRelu23(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 2048;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+4608;
    copy_k.loc = (uint32_t) l1_buffer + 29916;
    pi_cl_dma_memcpy(&copy_k);   
    pi_cl_dma_copy_t copy_lambda;
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 2048;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+6656;
    copy_lambda.loc = (uint32_t) l1_buffer + 31968;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  10752, // size: dimension of the buffer
  ...
  168, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  dory_dma_memcpy_3d_custom_blocking(
  l2_W, // ext
  (l1_buffer + 26888) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  1512, // size: dimension of matrix of weight * bytes_per_weight
  ...
  168, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  ...
  for(iter=0; iter<4*1*1; iter++) {
    ...
    last_nif_load = (_i_nof_load+1 == 4) ? 1 : 0;
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (last_nif_load) ? 8 : 168;
      x_tile_size_h   = (last_h_load)   ? 8 : 8;
      x_tile_size_w   = (last_w_load)   ? 8 : 8;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (last_nif_load)   ? 8 : 168;
      ...
      W_tile_size_nof = (last_nof_load) ? 8 : 168;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 8, 168, 8, 512,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 168.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 26888) + db_W, // loc
    ...
    k = (int32_t *) (l1_buffer + 29916 + _i_nof_exec*672);
    lambda = (int32_t *) (l1_buffer + 31968 + _i_nof_exec*672);
    W = (char *) (l1_buffer + 26888 + exec_db_W);
    y = (char *) (l1_buffer + 21508 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (last_nif_exec) ? 8 : 168;
    x_tile_size_h_exec   = (last_h_exec)   ? 8 : 8;
    x_tile_size_w_exec   = (last_w_exec)   ? 8 : 8;
    y_tile_size_nof = (last_nof_exec) ? 8 : 168;
    ...
    y_length_nof_byte = (last_nof_exec)   ? 8 : 168;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      // wait for DMA write/read
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 4, 4, 168, 4, 512, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 21508) + db_y, // loc
