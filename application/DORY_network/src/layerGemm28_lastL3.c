...
void __attribute__ ((noinline)) layerGemm28_lastL3(void *args) 
{
  ...
  L2_weights_1 = l2_W;
  L2_weights_2 = l2_W + 40960 + 0 + 0;
  transfer_weights = L2_weights_1;
  exec_weights = L2_weights_1;  
  // first tile transfer. Weights, k, lambda
  if(pi_core_id()==0)
  {
    pi_cl_ram_read(hyperram, l3_W, transfer_weights, 40960, &buff_req_w1);
  ...
  for(int k=0; k<25; k++) 
  {
    if (k < 24) 
    {
      if(pi_core_id()==0) 
      {
        pi_cl_ram_read(hyperram, (l3_W+(k+1)*40960), transfer_weights, 40960, &buff_req_w1);
      }
    }  
