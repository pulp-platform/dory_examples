...
void layerConvBNRelu0(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+864;
    copy_k.loc = (uint32_t) l1_buffer + 36210;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 256;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+1120;
    copy_lambda.loc = (uint32_t) l1_buffer + 36726;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  4947, // size: dimension of the buffer
  ...
  97,// length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 34478) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<1*1*2*8; iter++) {
    ...
    if(iter<1*1*2*8-1) 
    {
      ...
      x_tile_size_w   = (_i_w_load+1 == 8)   ? 17 : 17;
      ...
      y_tile_size_w   = (_i_w_load+1 == 8)   ? 8 : 8;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 97, 17, 3, 128, 3,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 3*3, 3, 3*3, 3, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 34478) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+864 + 256*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 36210 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+1120 + 256*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 36726 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 36210 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 36726 + exec_db_act);
    W = (char *) (l1_buffer + 34478 + exec_db_W);
    y = (char *) (l1_buffer + 9898 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 3 : 3;
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 33 : 97;
    x_tile_size_w_exec   = (_i_w_exec+1 == 8)   ? 17 : 17;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 32 : 32;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 16 : 48;
    y_tile_size_w   = (_i_w_exec+1 == 8)   ? 8 : 8;
    ...
    if(iter<1*1*2*8-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 48, 8, 32, 64, 32, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 9898) + db_y, // loc
