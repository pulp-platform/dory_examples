...
void layerConvBNRelu0(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 32058;
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+992;
    copy_lambda.loc = (uint32_t) l1_buffer + 32190;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  4407, // size: dimension of the buffer
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 30326) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<1*1*2*11; iter++) {
    ...
    db_x = !db_state_x ? 4407 : 0;
    db_W = !db_state_W ? 864 : 0;
    db_y = !db_state_y ? 10752 : 0;
    exec_db_x = db_state_x ? 4407 : 0;
    ...
    if(iter<1*1*2*11-1) 
    {
      ...
      x_tile_size_w   = (last_w_load)   ? 9 : 13;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 113, 13, 3, 128, 3,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
    ...
    k = (int32_t *) (l1_buffer + 32058 + _i_nof_exec*128);
    lambda = (int32_t *) (l1_buffer + 32190 + _i_nof_exec*128);
    W = (char *) (l1_buffer + 30326 + exec_db_W);
    y = (char *) (l1_buffer + 8818 + db_y);
    ...
    x_tile_size_w_exec   = (last_w_exec)   ? 9 : 13;
    ...
    pi_cl_team_barrier(0);
      ...
      mchan_barrier(dma_evt);
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 56, 6, 32, 64, 32, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 8818) + db_y, // loc
