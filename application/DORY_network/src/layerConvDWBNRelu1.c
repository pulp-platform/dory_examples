...
void layerConvDWBNRelu1(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+288;
    copy_k.loc = (uint32_t) l1_buffer + 37164;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 128;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+544;
    copy_lambda.loc = (uint32_t) l1_buffer + 37424;
  ...
  for(iter=0; iter<2*8*1; iter++) {
    ...
    if(iter<2*8*1-1) 
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 8)   ? 9 : 10;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 8)   ? 8 : 8;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 10, 64, 16, 64, 32,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+288 + 128*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 37164 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+544 + 128*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 37424 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 37164 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 37424 + exec_db_act);
    W = (char *) (l1_buffer + 36872 + exec_db_W);
    y = (char *) (l1_buffer + 20484 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 2) ? 16 : 16;
    x_tile_size_h_exec   = (_i_h_exec+1 == 8)   ? 9 : 10;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 64 : 64;
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 16 : 16;
    y_tile_size_h   = (_i_h_exec+1 == 8)   ? 8 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 1)   ? 64 : 64;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 2)   ? 16 : 16;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {  
      ...
      {                                       
        ...
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 64, 16, 64, 32, 0, 0, 0, 0, 0, 0, 8), // ext
