...
void layerConvDWBNRelu1(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+416;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  8704, // size: dimension of the buffer
  ...
  for(iter=0; iter<4*5*1; iter++) {
    ...
    db_x = !db_state_x ? 8704 : 0;
    db_W = !db_state_W ? 72 : 0;
    db_y = !db_state_y ? 7680 : 0;
    exec_db_x = db_state_x ? 8704 : 0;
    ...
    if(iter<4*5*1-1) 
    {
      ...
      x_tile_size_h   = (last_h_load)   ? 5 : 17;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (last_h_load)   ? 4 : 15;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 17, 64, 8, 64, 32,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8), // extern
    ...
    pi_cl_team_barrier(0);
      // wait for DMA write/read
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 15, 64, 8, 64, 32, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 17412) + db_y, // loc
