...
void layerConvBNRelu2(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 34828;
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 35856;
  ...
  for(iter=0; iter<1*1*2*4; iter++) {
    ...
    if(iter<1*1*2*4-1) 
    {
      ...
      x_tile_size_w   = (_i_w_load+1 == 4)   ? 4 : 20;
      ...
      y_tile_size_w   = (_i_w_load+1 == 4)   ? 4 : 20;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 20, 32, 64, 32,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.loc = (uint32_t) l1_buffer + 34828 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          ...
          copy_lambda.loc = (uint32_t) l1_buffer + 35856 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 34828 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 35856 + exec_db_act);
    ...
    x_tile_size_w_exec   = (_i_w_exec+1 == 4)   ? 4 : 20;
    y_tile_size_nof = (_i_nof_exec+1 == 1) ? 64 : 64;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 8 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 4)   ? 4 : 20;
    ...
    if(iter<1*1*2*4-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 20, 64, 64, 64, 0, 0, 0, 0, 0, 0, 8), // ext
