...
void layerConvBNRelu6(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 608;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+16384;
    copy_k.loc = (uint32_t) l1_buffer + 32524;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 608;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+17408;
    copy_lambda.loc = (uint32_t) l1_buffer + 33744;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 13064) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  9728, // size: dimension of matrix of weight * bytes_per_weight
  ...
  128, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  76, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  128, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  for(iter=0; iter<2*1*2*16; iter++) {
    ...
    db_W = !db_state_W ? 9728 : 0;
    db_y = !db_state_y ? 2432 : 0;
    db_act = !db_state_W ? 608 : 0;
    exec_db_x = db_state_x ? 4096 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 9728 : 0;
    exec_db_act = db_state_W ? 608 : 0;
    ...
    if(iter<2*1*2*16-1) 
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 2)   ? 16 : 16;
      x_tile_size_w   = (_i_w_load+1 == 16)   ? 2 : 2;
      ...
      y_tile_size_w   = (_i_w_load+1 == 16)   ? 2 : 2;
      W_tile_size_nof = (_i_nof_load+1 == 2) ? 52 : 76;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 16, 2, 128, 32, 128,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 76, 1*1, 128, 1*1, 128, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 13064) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+16384 + 608*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 32524 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+17408 + 608*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 33744 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 32524 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 33744 + exec_db_act);
    W = (char *) (l1_buffer + 13064 + exec_db_W);
    ...
    x_tile_size_h_exec   = (_i_h_exec+1 == 2)   ? 16 : 16;
    x_tile_size_w_exec   = (_i_w_exec+1 == 16)   ? 2 : 2;
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 52 : 76;
    y_tile_size_h   = (_i_h_exec+1 == 2)   ? 16 : 16;
    y_tile_size_w   = (_i_w_exec+1 == 16)   ? 2 : 2;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 2)   ? 52 : 76;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    if(iter<2*1*2*16-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 2, 76, 32, 128, 0, 0, 0, 0, 0, 0, 8), // ext
