...
void layerConvBNRelu6(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+16896;
  ...
  128, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  72, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  128, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  for(iter=0; iter<2*1*2*16; iter++) {
    ...
    if(iter<2*1*2*16-1) 
    {
      ...
      W_tile_size_nof = (last_nof_load) ? 56 : 72;
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 72, 1*1, 128, 1*1, 128, 0,0,0,0,0,0, 8), // ext
    ...
    k = (int32_t *) (l1_buffer + 31244 + _i_nof_exec*288);
    lambda = (int32_t *) (l1_buffer + 31760 + _i_nof_exec*288);
    ...
    y_tile_size_nof = (last_nof_exec) ? 56 : 72;
    ...
    y_length_nof_byte = (last_nof_exec)   ? 56 : 72;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      ...
      mchan_barrier(dma_evt);
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 2, 72, 32, 128, 0, 0, 0, 0, 0, 0, 8), // ext
