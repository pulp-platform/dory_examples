...
void layerConvDWBNRelu3_p_b(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+576;
    copy_k.loc = (uint32_t) l1_buffer + 35404;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 256;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+1088;
    copy_lambda.loc = (uint32_t) l1_buffer + 35920;
  ...
  l2_W, // ext
  (l1_buffer + 34824) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<2*3*1; iter++) {
    ...
    if(iter<2*3*1-1) 
    {
      ...
      x_tile_size_h   = (_i_h_load+1 == 3)   ? 5 : 7;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 3)   ? 2 : 3;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 7, 64, 32, 64, 64,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+576 + 256*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 35404 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+1088 + 256*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 35920 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 35404 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 35920 + exec_db_act);
    W = (char *) (l1_buffer + 34824 + exec_db_W);
    y = (char *) (l1_buffer + 28676 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 2) ? 32 : 32;
    x_tile_size_h_exec   = (_i_h_exec+1 == 3)   ? 5 : 7;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 64 : 64;
    y_tile_size_nof = (_i_nof_exec+1 == 2) ? 32 : 32;
    y_tile_size_h   = (_i_h_exec+1 == 3)   ? 2 : 3;
    y_tile_size_w   = (_i_w_exec+1 == 1)   ? 32 : 32;
    ...
    {  
      ...
      {                                       
        ...
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 3, 32, 32, 32, 64, 0, 0, 0, 0, 0, 0, 8), // ext
