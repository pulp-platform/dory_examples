...
void layerGemm28_last(
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  1024, // size: dimension of the buffer
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 1096) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<5*1*1*1; iter++) {
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 1*1, 1024, 1*1, 1024, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 1096) + db_W, // loc
    ...
    W = (char *) (l1_buffer + 1096 + exec_db_W);
    y = (char *) (l1_buffer + 1028 + db_y);
    ...
    pi_cl_team_barrier(0);
      ...
      mchan_barrier(dma_evt);
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 1, 1, 8, 1, 1000, 0, 0, 0, 0, 0, 0, 32), // ext
        (l1_buffer + 1028) + db_y, // loc
