...
void layerConvBNRelu26(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 16428;
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+65792;
    copy_lambda.loc = (uint32_t) l1_buffer + 16688;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 8232) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<16*1*1*4; iter++) {
    ...
    if(iter<16*1*1*4-1) 
    {
      x_tile_size_nif = (last_nif_load) ? 1024 : 1024;
      x_tile_size_h   = (last_h_load)   ? 4 : 4;
      x_tile_size_w   = (last_w_load)   ? 1 : 1;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (last_nif_load)   ? 1024 : 1024;
      ...
      W_tile_size_nif = (last_nif_load) ? 1024 : 1024;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*1*1/8;
      W_length_nif_byte = (last_nif_load) ? 1024 : 1024;
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 4, 1, 1024, 4, 1024,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8), // extern
    ...
    k = (int32_t *) (l1_buffer + 16428 + _i_nof_exec*16);
    lambda = (int32_t *) (l1_buffer + 16688 + _i_nof_exec*16);
    W = (char *) (l1_buffer + 8232 + exec_db_W);
    ...
    pi_cl_team_barrier(0);
      ...
      mchan_barrier(dma_evt);
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 4, 1, 4, 4, 1024, 0, 0, 0, 0, 0, 0, 8), // ext
