...
void layerConvBNRelu26(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+4096;
    copy_k.loc = (uint32_t) l1_buffer + 20556;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 32;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+4128;
    copy_lambda.loc = (uint32_t) l1_buffer + 20592;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 16456) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  4096, // size: dimension of matrix of weight * bytes_per_weight
  1024, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  1024, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  4, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  1024, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    if(iter<1*1*1*1-1) 
    {
      ...
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 1024 : 1024;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*1*1/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 1024 : 1024;
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 4, 1*1, 1024, 1*1, 1024, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 16456) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+4096 + 32*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 20556 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+4128 + 32*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 20592 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 20556 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 20592 + exec_db_act);
    W = (char *) (l1_buffer + 16456 + exec_db_W);
    y = (char *) (l1_buffer + 16388 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 1024 : 1024;
    ...
    if(iter<1*1*1*1-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 4, 4, 4, 4, 1024, 0, 0, 0, 0, 0, 0, 8), // ext
