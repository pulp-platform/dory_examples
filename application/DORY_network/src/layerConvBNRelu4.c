...
void layerConvBNRelu4(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 28684;
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 30736;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 12296) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<1*1*2*16; iter++) {
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 16, 2, 64, 32, 64,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.loc = (uint32_t) l1_buffer + 28684 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          ...
          copy_lambda.loc = (uint32_t) l1_buffer + 30736 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 28684 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 30736 + exec_db_act);
    W = (char *) (l1_buffer + 12296 + exec_db_W);
    ...
    if(iter<1*1*2*16-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 2, 128, 32, 128, 0, 0, 0, 0, 0, 0, 8), // ext
