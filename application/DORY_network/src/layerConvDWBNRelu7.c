...
void layerConvDWBNRelu7(
  ...
  // last-tile flags
  int last_nof_load = (11 == 1) ? 1 : 0;
  int last_nif_load = (11 == 1) ? 1 : 0;
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+1152;
    copy_k.loc = (uint32_t) l1_buffer + 30948;
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+1664;
    copy_lambda.loc = (uint32_t) l1_buffer + 31464;
  ...
  12, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  108, // size: dimension of matrix of weight * bytes_per_weight
  ...
  for(iter=0; iter<11*1*1; iter++) {
    ...
    last_nof_load = (_i_nof_load+1 == 11) ? 1 : 0;
    last_nif_load = (_i_nof_load+1 == 11) ? 1 : 0;
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (last_nif_load) ? 8 : 12;
      ...
      x_length_nif_byte = (last_nif_load)   ? 8 : 12;
      ...
      W_tile_size_nof = (last_nof_load) ? 8 : 12;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 32, 32, 12, 32, 128,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 12.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
    ...
    y_length_nof_byte = (last_nof_exec)   ? 8 : 12;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      // wait for DMA write/read
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 16, 16, 12, 16, 128, 0, 0, 0, 0, 0, 0, 8), // ext
