...
void layerConvDWBNRelu13(
  ...
  // last-tile flags
  int last_nof_load = (5 == 1) ? 1 : 0;
  int last_nif_load = (5 == 1) ? 1 : 0;
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 30700;
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 32752;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  7168, // size: dimension of the buffer
  ...
  112, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  1008, // size: dimension of matrix of weight * bytes_per_weight
  ...
  112, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  ...
  for(iter=0; iter<5*1*1; iter++) {
    ...
    last_nof_load = (_i_nof_load+1 == 5) ? 1 : 0;
    last_nif_load = (_i_nof_load+1 == 5) ? 1 : 0;
    last_h_load = (_i_h_load+1 == 1) ? 1 : 0;
    last_w_load = (_i_w_load+1 == 1) ? 1 : 0;

    // compute double buffering offsets and update db state
    db_x = !db_state_x ? 7168 : 0;
    db_W = !db_state_W ? 1008 : 0;
    db_y = !db_state_y ? 7168 : 0;
    exec_db_x = db_state_x ? 7168 : 0;
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (last_nif_load) ? 64 : 112;
      ...
      x_length_nif_byte = (last_nif_load)   ? 64 : 112;
      ...
      W_tile_size_nof = (last_nof_load) ? 64 : 112;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 8, 112, 8, 512,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 112.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
    ...
    y_length_nof_byte = (last_nof_exec)   ? 64 : 112;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      // wait for DMA write/read
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 8, 112, 8, 512, 0, 0, 0, 0, 0, 0, 8), // ext
