...
void layerConvDWBNRelu13(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 960;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+4608;
    copy_k.loc = (uint32_t) l1_buffer + 32892;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 960;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+8704;
    copy_lambda.loc = (uint32_t) l1_buffer + 34816;
  ...
  l2_x, // ext
  (l1_buffer + 0) + 0, // loc
  7680, // size: dimension of the buffer
  ...
  512, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  8,// length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  120, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  dory_dma_memcpy_3d_custom_blocking(
  l2_W, // ext
  (l1_buffer + 30728) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  1080, // size: dimension of matrix of weight * bytes_per_weight
  ...
  120, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  ...
  for(iter=0; iter<5*1*1; iter++) {
    ...
    db_W = !db_state_W ? 1080 : 0;
    db_y = !db_state_y ? 7680 : 0;
    db_act = !db_state_W ? 960 : 0;
    exec_db_x = db_state_x ? 7680 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 1080 : 0;
    exec_db_act = db_state_W ? 960 : 0;
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 5) ? 32 : 120;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 5)   ? 32 : 120;
      ...
      W_tile_size_nof = (_i_nof_load+1 == 5) ? 32 : 120;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 8, 120, 8, 512,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      {
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_W, _i_nof_load, 0, 0, 120.0, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 30728) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+4608 + 960*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 32892 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+8704 + 960*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 34816 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 32892 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 34816 + exec_db_act);
    W = (char *) (l1_buffer + 30728 + exec_db_W);
    y = (char *) (l1_buffer + 15364 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 5) ? 32 : 120;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 8 : 8;
    x_tile_size_w_exec   = (_i_w_exec+1 == 1)   ? 8 : 8;
    y_tile_size_nof = (_i_nof_exec+1 == 5) ? 32 : 120;
    y_tile_size_h   = (_i_h_exec+1 == 1)   ? 8 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 1)   ? 8 : 8;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (_i_nof_exec+1 == 5)   ? 32 : 120;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    {  
      ...
      {                                       
        ...
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 8, 120, 8, 512, 0, 0, 0, 0, 0, 0, 8), // ext
        (l1_buffer + 15364) + db_y, // loc
