...
void layerConvBNRelu12(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 288;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+131072;
    copy_k.loc = (uint32_t) l1_buffer + 27788;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 288;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+135168;
    copy_lambda.loc = (uint32_t) l1_buffer + 28368;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 9352) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  256, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  36, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  256, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  for(iter=0; iter<15*1*1*4; iter++) {
    ...
    if(iter<15*1*1*4-1) 
    {
      ...
      W_tile_size_nof = (_i_nof_load+1 == 15) ? 8 : 36;
    ...
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 2, 256, 8, 256,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 36, 1*1, 256, 1*1, 256, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 9352) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+131072 + 288*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 27788 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+135168 + 288*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 28368 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 27788 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 28368 + exec_db_act);
    W = (char *) (l1_buffer + 9352 + exec_db_W);
    ...
    y_tile_size_nof = (_i_nof_exec+1 == 15) ? 8 : 36;
    ...
    y_length_nof_byte = (_i_nof_exec+1 == 15)   ? 8 : 36;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    if(iter<15*1*1*4-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 36, 8, 512, 0, 0, 0, 0, 0, 0, 8), // ext
