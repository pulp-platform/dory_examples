...
void layerConvBNRelu12(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 25612;
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+133120;
    copy_lambda.loc = (uint32_t) l1_buffer + 27664;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 9224) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  ...
  for(iter=0; iter<16*1*1*4; iter++) {
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 1*1, 256, 1*1, 256, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 9224) + db_W, // loc
    ...
    k = (int32_t *) (l1_buffer + 25612 + _i_nof_exec*128);
    lambda = (int32_t *) (l1_buffer + 27664 + _i_nof_exec*128);
    W = (char *) (l1_buffer + 9224 + exec_db_W);
    ...
    pi_cl_team_barrier(0);
      ...
      mchan_barrier(dma_evt);
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 32, 8, 512, 0, 0, 0, 0, 0, 0, 8), // ext
