...
void layerConvBNRelu24(
  ...
  16384, // size: dimension of matrix of weight * bytes_per_weight
  ...
  for(iter=0; iter<1*1*1*1; iter++) {
    ...
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 32, 1*1, 512, 1*1, 512, 0,0,0,0,0,0, 8), // ext
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+16384 + 256*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 25100 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+16640 + 256*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 25360 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 25100 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 25360 + exec_db_act);
    ...
    if(iter<1*1*1*1-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 4, 4, 32, 4, 1024, 0, 0, 0, 0, 0, 0, 8), // ext
