...
void layerConvBNRelu10(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 28816;
  ...
  for(iter=0; iter<8*1*2*8; iter++) {
    ...
    if(iter<8*1*2*8-1) 
    {
      x_tile_size_nif = (last_nif_load) ? 256 : 256;
      x_tile_size_h   = (last_h_load)   ? 8 : 8;
      x_tile_size_w   = (last_w_load)   ? 2 : 2;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (last_nif_load)   ? 256 : 256;
      ...
      y_tile_size_w   = (last_w_load)   ? 2 : 2;
      W_tile_size_nof = (last_nof_load) ? 4 : 36;
      W_tile_size_nif = (last_nif_load) ? 256 : 256;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*1*1/8;
      W_length_nif_byte = (last_nif_load) ? 256 : 256;
    ...
    k = (int32_t *) (l1_buffer + 27788 + _i_nof_exec*144);
    lambda = (int32_t *) (l1_buffer + 28816 + _i_nof_exec*144);
    W = (char *) (l1_buffer + 9352 + exec_db_W);
    y = (char *) (l1_buffer + 8196 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (last_nif_exec) ? 256 : 256;
    x_tile_size_h_exec   = (last_h_exec)   ? 8 : 8;
    x_tile_size_w_exec   = (last_w_exec)   ? 2 : 2;
    y_tile_size_nof = (last_nof_exec) ? 4 : 36;
    y_tile_size_h   = (last_h_exec)   ? 8 : 8;
    y_tile_size_w   = (last_w_exec)   ? 2 : 2;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (last_nof_exec)   ? 4 : 36;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      ...
      mchan_barrier(dma_evt);
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 36, 16, 256, 0, 0, 0, 0, 0, 0, 8), // ext
