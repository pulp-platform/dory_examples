...
void layerConvDWBNRelu5(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 32924;
    ...
    copy_lambda.loc = (uint32_t) l1_buffer + 33056;
  ...
  for(iter=0; iter<16*1*1; iter++) {
    ...
    {
      asm volatile("": : :"memory");
      x_tile_size_nif = (_i_nif_load+1 == 16) ? 8 : 8;
      ...
      x_length_nif_byte = (_i_nif_load+1 == 16)   ? 8 : 8;
      ...
      W_tile_size_nif = (_i_nif_load+1 == 16) ? 1 : 1;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*3*3;
      W_length_nif_byte = (_i_nif_load+1 == 16) ? 1 : 1;
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom_hwc_to_chw(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 32, 32, 8, 32, 128,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+1152 + 64*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 32924 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+2176 + 64*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 33056 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    {  
      ...
      {                                       
        ...
        dory_dma_memcpy_3d_custom_blocking(
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 32, 32, 8, 32, 128, 0, 0, 0, 0, 0, 0, 8), // ext
