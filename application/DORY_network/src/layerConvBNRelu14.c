...
void layerConvBNRelu14(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.merge = 0;
    copy_k.size = (uint16_t) 96;
    copy_k.id = 0;
    copy_k.ext = (uint32_t) l2_W+65536;
    copy_k.loc = (uint32_t) l1_buffer + 29068;
    pi_cl_dma_memcpy(&copy_k);   
    copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
    copy_lambda.merge = 0;
    copy_lambda.size = (uint16_t) 96;
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+66560;
    copy_lambda.loc = (uint32_t) l1_buffer + 29264;
  ...
  512, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 16776) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  6144, // size: dimension of matrix of weight * bytes_per_weight
  512, // stride_1: stride for the 3d copy: if we have to copy on n_features axis, this is the stride to change from first 2D space to the next ones.
  512, // stride_0: stride to be passed to 2d_copy: the dimension w of the in image
  12, // length_2: how many 2_d copies we need -> the dimension of the tile in n_features direction
  512, // length_0: legnth of the 1_d copy, the length of tile in w direction
  1, // dir
  &dma_evt // copy
  );
  ...
  for(iter=0; iter<11*1*1*4; iter++) {
    ...
    db_W = !db_state_W ? 6144 : 0;
    db_y = !db_state_y ? 192 : 0;
    db_act = !db_state_W ? 96 : 0;
    exec_db_x = db_state_x ? 8192 : 0;
    db_state_x = ! db_state_x;
    exec_db_W = db_state_W ? 6144 : 0;
    ...
    if(iter<11*1*1*4-1) 
    {
      x_tile_size_nif = (_i_nif_load+1 == 1) ? 512 : 512;
      x_tile_size_h   = (_i_h_load+1 == 1)   ? 8 : 8;
      x_tile_size_w   = (_i_w_load+1 == 4)   ? 2 : 2;
      x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
      x_length_nif_byte = (_i_nif_load+1 == 1)   ? 512 : 512;
      ...
      W_tile_size_nif = (_i_nif_load+1 == 1) ? 512 : 512;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*1*1/8;
      W_length_nif_byte = (_i_nif_load+1 == 1) ? 512 : 512;
    // transfer of next input tile in double buffering
      dory_dma_memcpy_3d_custom(
      dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 8, 2, 512, 8, 512,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8), // extern
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
      {
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 12, 1*1, 512, 1*1, 512, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 16776) + db_W, // loc
        ...
        if(pi_core_id()==0)
        {
          ...
          copy_k.id = 0;
          copy_k.ext = (uint32_t) l2_W+65536 + 96*_i_nof_load;
          copy_k.loc = (uint32_t) l1_buffer + 29068 + db_act;
          pi_cl_dma_memcpy(&copy_k);   
          copy_lambda.dir = PI_CL_DMA_DIR_EXT2LOC;
          copy_lambda.merge = 0;
          copy_lambda.size = (uint16_t) W_tile_size_nof * 8;
          copy_lambda.id = 0;
          copy_lambda.ext = (uint32_t) l2_W+66560 + 96*_i_nof_load;
          copy_lambda.loc = (uint32_t) l1_buffer + 29264 + db_act;
          pi_cl_dma_memcpy(&copy_lambda);      
        }
      }
    }
    ...
    k = (int64_t *) (l1_buffer + 29068 + exec_db_act);
    lambda = (int64_t *) (l1_buffer + 29264 + exec_db_act);
    W = (char *) (l1_buffer + 16776 + exec_db_W);
    y = (char *) (l1_buffer + 16388 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (_i_nif_exec+1 == 1) ? 512 : 512;
    x_tile_size_h_exec   = (_i_h_exec+1 == 1)   ? 8 : 8;
    x_tile_size_w_exec   = (_i_w_exec+1 == 4)   ? 2 : 2;
    y_tile_size_nof = (_i_nof_exec+1 == 11) ? 8 : 12;
    y_tile_size_h   = (_i_h_exec+1 == 1)   ? 8 : 8;
    y_tile_size_w   = (_i_w_exec+1 == 4)   ? 2 : 2;
    ...
    if(iter<11*1*1*4-1) 
    {
      ...
      {                                       
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 12, 8, 512, 0, 0, 0, 0, 0, 0, 8), // ext
