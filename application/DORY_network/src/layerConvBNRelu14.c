...
void layerConvBNRelu14(
  ...
  if(pi_core_id()==0)
  {
    ...
    copy_k.loc = (uint32_t) l1_buffer + 24844;
    ...
    copy_lambda.id = 0;
    copy_lambda.ext = (uint32_t) l2_W+66048;
    copy_lambda.loc = (uint32_t) l1_buffer + 25360;
  ...
  dory_dma_memcpy_3d_custom(
  l2_W, // ext
  (l1_buffer + 16648) + 0, // loc offset caused by size of tile_x*2 (double_buffer) and tile_y*2 (double buffer)
  4096, // size: dimension of matrix of weight * bytes_per_weight
  ...
  for(iter=0; iter<16*1*1*4; iter++) {
    ...
    if(iter<16*1*1*4-1) 
    {
      x_tile_size_nif = (last_nif_load) ? 512 : 512;
      ...
      x_length_nif_byte = (last_nif_load)   ? 512 : 512;
      ...
      W_tile_size_nif = (last_nif_load) ? 512 : 512;
      W_tile_size_byte = W_tile_size_nof*W_tile_size_nif*8*1*1/8;
      W_length_nif_byte = (last_nif_load) ? 512 : 512;
    // transfer of next input tile in double buffering
      ...
      if (_i_nif_load!=_i_nif_exec || _i_nof_load!=_i_nof_exec)
        ...
        dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 8, 1*1, 512, 1*1, 512, 0,0,0,0,0,0, 8), // ext
        (l1_buffer + 16648) + db_W, // loc
    ...
    k = (int32_t *) (l1_buffer + 24844 + _i_nof_exec*32);
    lambda = (int32_t *) (l1_buffer + 25360 + _i_nof_exec*32);
    W = (char *) (l1_buffer + 16648 + exec_db_W);
    y = (char *) (l1_buffer + 16388 + db_y);
    // parameter passed to the kernel. Input and output sizes
    x_tile_size_nif_exec = (last_nif_exec) ? 512 : 512;
    ...
    pi_cl_team_barrier(0);
      ...
      mchan_barrier(dma_evt);
        ...
        dory_get_tile_3d(l2_y, _i_h_exec, _i_w_exec, _i_nof_exec, 8, 2, 8, 8, 512, 0, 0, 0, 0, 0, 0, 8), // ext
