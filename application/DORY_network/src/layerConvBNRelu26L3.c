...
void __attribute__ ((noinline)) layerConvBNRelu26L3(void *args) 
{
  ...
  L2_weights_1 = l2_W;
  L2_weights_2 = l2_W + 65536 + 256 + 256;
  ...
  if(pi_core_id()==0)
  {
    ...
    pi_cl_ram_read(hyperram, l3_W+1048576, transfer_weights + 65536, 256, &buff_req_w2);
    pi_cl_ram_read(hyperram, l3_W+1052672, transfer_weights + 65536 + 256, 256, &buff_req_w3);
  ...
  for(int k=0; k<16; k++) 
  {
    if (k < 15) 
    {
      if(pi_core_id()==0) 
      {
        ...
        pi_cl_ram_read(hyperram, l3_W+1048576+ (k+1)*256, transfer_weights + 65536, 256, &buff_req_w2);
        pi_cl_ram_read(hyperram, l3_W+1052672 + (k+1)*256, transfer_weights + 65536+ 256, 256, &buff_req_w3);
      }
    }  
    ...
    if (j==0)
    {
      unsigned int args[13] = {l3_x,
          ...
          exec_input,
          l2_x_2,
          dory_get_tile_3d(exec_output, 0, 0, k, 4, 4, 64, 4, 1024, 0, 0, 0, 0, 0, 0, 8),
          exec_weights,
          l1_buffer,
          hyperram,
          outmult,
    ...
    else if (j==(0))
    {
      unsigned int args[13] = {l3_x,
          ...
          exec_input,
          l2_x_2,
          dory_get_tile_3d(exec_output, j, 0, k, 4, 4, 64, 4, 1024, 0, 0, 0, 0, 0, 0, 8),
          exec_weights,
          l1_buffer,
          hyperram,
          outmult,
