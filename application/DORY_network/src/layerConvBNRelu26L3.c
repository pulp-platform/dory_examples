...
void __attribute__ ((noinline)) layerConvBNRelu26L3(void *args) 
{
  ...
  if(pi_core_id()==0)
  {
    ...
    pi_cl_ram_read(hyperram, l3_W+1048576, transfer_weights + 4096, 32, &buff_req_w2);
    pi_cl_ram_read(hyperram, l3_W+1056768, transfer_weights + 4096 + 32, 32, &buff_req_w3);
  ...
  for(int k=0; k<256; k++) 
  {
    if (k < 255) 
    {
      if(pi_core_id()==0) 
      {
        ...
        pi_cl_ram_read(hyperram, l3_W+1048576+ (k+1)*32, transfer_weights + 4096, 32, &buff_req_w2);
        pi_cl_ram_read(hyperram, l3_W+1056768 + (k+1)*32, transfer_weights + 4096+ 32, 32, &buff_req_w3);
      }
    }  
